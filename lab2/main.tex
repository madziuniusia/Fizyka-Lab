...
\grupa{14}
\zespol{2}
\temat{Modelowanie pola elektrycznego}
\nrcwiczenia{31}
\datawykonania{17.10.2025}
\dataoddaniajeden{24.10.2025}
\zwrotdopoprawy{}
\dataoddaniadwa{}
\datazaliczenia{}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.8\linewidth]{wykres_plaski.png}
...
$r_{\text{wew}} = 0{,}019$ m. Wartości doświadczalne potencjału
$V_{\text{dośw}}$ zostały porównane z wartościami teoretycznymi
$V_{\text{teor}}$, które zostały wyznaczone za pomocą wzoru (8).
\begin{table}[h]
    \centering
...
Niepewności typu B wynikające z dokładności urządzeń pomiarowych
zostały uwzględnione w wynikach przedstawionych w tabeli 1 i
tabeli 2. Zwiększona wartość niepewności względem dokładności podanych w opisie aparatury wynika z uwzględnienia błędu ludzkiego podczas odczytu wartości oraz trudności związanych z operowaniem używanym multimetrem. Wynoszą one:
\begin{equation}
u_B(x) = 0{,}001\,\text{m}
\label{eq:uB_x}
\end{equation}
...
Niepewność $E_{\text{dośw}}$ danego wzorem (4) można wyznaczyć korzystając z zasady przenoszenia niepewności, przyjmując dla każdego pomiaru $n$:
\begin{equation}
u(V_n) = u(V_{n+1}) = u_B(V) \quad \text{oraz} \quad u(x_n) = u(x_{n+1}) = u_B(x)
\label{eq:niepewnosci}
\end{equation}
\begin{equation}
u(E_{\text{dośw}}) = \sqrt{2 \cdot \left(\frac{u(V)}{x_{n+1} - x_n}\right)^2 + 2 \cdot \left(\frac{(x_{n+1} - x_n)^2 \cdot u(x)}{V_{n+1} - V_n}\right)^2}
\label{eq:u_Edosw}
\end{equation}

\pagebreak
\section{Wyznaczenie natężenia E pola elektrycznego}
Dla sąsiednich współliniowych punktów wewnątrz kondensatora cylindrycznego i płaskiego średnie natężenie pola wyznacza się na podstawie dwóch kolejnych zmierzonych wartości potencjału w punktach $n$ oraz $n+1$. Otrzymaną wartość przypisuje się punktowi leżącemu w połowie odległości między tymi punktami, tzn. dla:
\begin{equation}
x^* = \frac{x_n + x_{n+1}}{2}
\label{eq:x_star}
\end{equation}
...
\begin{equation}
E_{\text{dośw}} = \frac{V_{n+1} - V_n}{x_{n+1} - x_n}
\label{eq:Edosw}
\end{equation}
...
\hline
1 & 0.0505 & 190.00 & 191.03 \\
\hline
2 & 0.0575 & 144.29 & 167.76 \\
\hline
3 & 0.0645 & 115.71 & 149.58 \\
\hline
4 & 0.0715 & 82.86 & 134.92 \\
\hline
5 & 0.0785 & 82.86 & 122.90 \\
\hline
6 & 0.0855 & 74.29 & 112.82 \\
\hline
7 & 0.0925 & 72.86 & 104.29 \\
\hline
8 & 0.0995 & 62.86 & 96.95 \\
\hline
9 & 0.1065 & 52.86 & 90.58 \\
\hline
\end{tabular}
...
Doświadczenie wykazało, że wartości potencjału oraz natężenia pola elektrycznego
wewnątrz kondensatora płaskiego pokrywają się z wartościami teoretycznymi,
chociaż występują znaczące niedokładności.
Dla kondensatora cylindrycznego pomiary w znaczący sposób odbiegają od oczekiwanych
wartości. Może być to spowodowane wadliwymi płytkami modelowymi, szczególnie przy
pomiarze c lub nieodpowiednim dociskiem sondy podczas zbierania pomiarów.

Eksperyment pokazał, że natężenie pola wewnątrz kondensatora cylindrycznego
zmienia się nieliniowo. Dodatkowo dla kondensatora płaskiego, linie pola układają
się w charakterystyczne półokręgi i przypominają swoim kształtem pole magnetyczne
magnesu podkowiastego.
